...
async function editRecord(type,id){const list=type==="work"?state.data.work_sessions:type==="driving"?state.data.driving_sessions:state.data.fuel_fillups,x=list.find(r=>r.id===id);if(!x)return;let body={...x};
 if(type==="work"){body.started_at=prompt("Début ISO",x.started_at)||x.started_at;body.ended_at=prompt("Fin ISO (vide = en cours)",x.ended_at||"")||null}
 if(type==="driving"){body.vehicle_registration=prompt("Véhicule",x.vehicle_registration)||x.vehicle_registration;body.km_start=prompt("Km départ",x.km_start);body.km_end=prompt("Km fin",x.km_end??"");body.started_at=prompt("Début ISO",x.started_at)||x.started_at;body.ended_at=prompt("Fin ISO",x.ended_at||"")||null}
 if(type==="fuel"){body.odometer_km=prompt("Kilométrage",x.odometer_km);body.litres=prompt("Litres",x.litres);body.filled_at=prompt("Date ISO",x.filled_at)||x.filled_at}
 await api(`/api/timeclock/records/${type}/${id}`,{method:"PATCH",body:JSON.stringify(body)});await load()}
